...
def get_loaders(dataset, batch_size, path_to_data, num_workers, num_sel_train_images=-1, num_sel_test_images=-1):
    train_loader = get_train_loader(dataset, batch_size, path_to_data, num_workers, num_sel_images=num_sel_train_images)
    val_loader  = get_val_loader( dataset, batch_size, path_to_data, num_workers, num_sel_images=num_sel_test_images)
...
def get_train_loader(dataset, batch_size, path_to_data, num_workers, num_sel_images):
    ...
    if dataset == 'MNIST':
        train_data = MNIST(path_to_data, train=True, download=True, transform=train_transform)
