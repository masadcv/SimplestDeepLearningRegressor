...
def train(args, model, train_loader, optimizer, loss_func, epoch):
    ...
    args['tb_writer'].add_scalar('train_%s_loss' %
                                 args['epoch_name'], losses.avg, epoch)
...
def evaluate(args, model, test_loader, loss_func, epoch):
    ...
    args['tb_writer'].add_scalar('test_%s_loss' %
                                 args['epoch_name'], losses.avg, epoch)
    args['tb_writer'].add_scalar('test_%s_acc' %
                                 args['epoch_name'], accuracy, epoch)
